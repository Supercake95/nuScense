...
class NuScenes:
    ...
    def render_pointcloud_in_image(self, dims: int, sample_token: str, dot_size: int = 20, pointsensor_channel: str = 'LIDAR_TOP',
                                   camera_channel: str = 'CAM_FRONT', out_path: str = None,
                                   render_intensity: bool = False) -> None:
        self.explorer.render_pointcloud_in_image(dims, sample_token, dot_size, pointsensor_channel=pointsensor_channel,
...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image(self,
        ...
        if render_intensity:
            ...
            for i in range(0,18):
                if dims==i:
                    clo = pc.points[i, :]
                    clo = (clo - np.min(clo)) / (np.max(clo) - np.min(clo))
                    clo = clo * 255
    ...
    def render_pointcloud_in_image(self,
        ...
        points, coloring, im = self.map_pointcloud_to_image(dims,pointsensor_token, camera_token,
                                                            render_intensity=render_intensity)
